...
| `Application` `travel-*` `OutOfSync` com `Healthy` | os workloads são criados pelo `provision.sh` a partir de `platform-reference/workloads/`, **sem anotação de rastreio do Argo**, e o `ApplicationSet rhcl-travel` reivindica os mesmos objetos a partir dos repositórios semeados. Sem auto-sync e sem `selfHeal`, o Argo vê diferença e não age — é a fronteira de governança funcionando, e o preflight a afirma ("camada de demo continua fora do controle do Argo"). Sincronizar faria o Argo virar dono dos workloads da demo, que é o oposto da decisão |
| `Application` `travel-travel-packages` `Missing` com `SyncError` | o projeto traz o `travel-packages` em JBoss EAP, e o namespace só existe depois de `provision.sh pacotes` — a rota longa (+54 min, mais o build da imagem) que este ambiente não executa. Apagar a `Application` não resolve: o `ApplicationSet` a recria em ~3 min. Ou se roda `pacotes`, ou se aceita o `Missing` |
